...
class DefaultSOTAdapter(BaseSOTAdapter):
    ...
    def __init__(self, model_adapter, dataset, state_dir, tensor_version_interval, hyperparams_getter=None):
        ...
        # Instead of version_calculation_lock, rename it for concurrency safety:
        self.sot_state_lock = asyncio.Lock()
    ...
    async def initialize(self, sot_id, db_url, private_key, job_id, perm_db, port):
        ...
        job_obj = await self.db_adapter.get_job_sot(self.job_id)
        ...
        # ---------------------- KEY FIX: Updated get_batch_route ----------------------
        ...
        async def get_batch_route():
            ...
            # fetch the dict or None
            result_dict = await self.get_batch()
            if result_dict is None:
                # Return a JSON error if no data
        ...
        async def update_state():
            ...
            logging.info(f"[update_state] Received result_url: {result_url} and base_dir: {self.base_dir}")

            tensor_name = TENSOR_NAME
            # Just read state, no update to it here:
            ...
            logging.info(f"[update_state] Computed local_file_path: {os.path.abspath(local_file_path)} (exists: {os.path.exists(local_file_path)}) from result_url: {result_url} with base_dir: {self.base_dir}")
            ...
            if os.path.exists(self.aggregator_sum_path):
                ...
                aggregator_sum = torch.zeros_like(partial_grads)
        ...
        async def get_diffs_since():
            ...
            # sort ascending by the version number embedded in the URL
            diffs_list = sorted(
                diffs_list,
                key=lambda url: int(url.split('_')[-1].split('.')[0])  # naive parse
            )
        ...
        async def latest_state():
            ...
            tensor_name = TENSOR_NAME
            async with self.sot_state_lock:
                state_data = await self.db_adapter.get_sot_state_for_job(self.job_id)
                block_timestamps = state_data.get("block_timestamps", {})
    ...
    async def _cleanup_old_files(self):
        cutoff_seconds = 2 * 24 * 3600  # 48 hours
        now = time.time()
    ...
    # ---------------------- KEY FIX: Updated get_batch method ----------------------
    async def get_batch(self):
        """
        Returns a dict of torch.Tensors for the next batch, or None if no data is available.
        """
        logging.info("[get_batch] Worker requests a batch.")
        try:
            token_pairs = await self.dataset.__anext__()  # e.g. list of (input, target)
            if not token_pairs:
                logging.info("[get_batch] No more data in dataset.")
                return None
