...
async def initialize_tensor(name, state_dir, tensor_version_interval, init_tensor_func, zero_init=False, memory_logging=False, file_locks=None):
    ...
    sync_version_number = block_timestamps.get(
        name, get_current_version_number(tensor_version_interval))
...
async def initialize_all_tensors(state_dir, tensor_version_interval, init_tensor_func, memory_logging=False, file_locks=None):
    logging.info("Initializing all tensors")
    # Pass file_locks to initialize_tensor
    await initialize_tensor(TENSOR_NAME, state_dir, tensor_version_interval, init_tensor_func, zero_init=False, memory_logging=memory_logging, file_locks=file_locks)
    await initialize_tensor(f'{TENSOR_NAME}_adam_m', state_dir, tensor_version_interval, init_tensor_func, zero_init=True, memory_logging=memory_logging, file_locks=file_locks)

async def fix_outdated_last_future_version_number(tensor_name, last_future_version_number, state_dir, tensor_version_interval):
    value = last_future_version_number.get(tensor_name, 0)
    current_version_number = get_current_version_number(tensor_version_interval)
...
async def update_block_timestamps(tensor_name, block_timestamps, num_updates, iteration_number, last_future_version_number, state_dir, db_adapter, job_id, tensor_version_interval, update_timestamp_lock, file_locks):
    async with update_timestamp_lock:
        future_version_number = get_future_version_number(tensor_version_interval)
        await fix_outdated_last_future_version_number(tensor_name, last_future_version_number, state_dir, tensor_version_interval)
...
async def update_cleanup_timestamps(tensor_name, block_timestamps, num_updates, iteration_number, last_future_version_number, state_dir, db_adapter, job_id, tensor_version_interval, update_timestamp_lock, file_locks):
    old_block_timestamp = await update_block_timestamps(
        tensor_name, block_timestamps, num_updates, iteration_number,
        last_future_version_number, state_dir, db_adapter, job_id, tensor_version_interval, update_timestamp_lock, file_locks
    )
