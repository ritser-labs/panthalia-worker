...
class ModelAdapter:
    ...
    def train_step(self, data, targets, lr, weight_decay, epochs):
        raise NotImplementedError("This method should be implemented by subclasses.")
...
class SimpleLinearAdapter(ModelAdapter):
    ...
    def train_step(self, data, targets, lr, weight_decay, epochs):
        ...
        for epoch in range(epochs):
            ...
            params, m, v = stable_adamw_update(params, grads, m, v, lr, weight_decay, step=epoch+1)
...
class TransformerLLMAdapter(ModelAdapter):
    ...
    def train_step(self, data, targets, lr, weight_decay, epochs):
        ...
        for epoch in range(epochs):
            ...
            outputs = self.model(data, start_pos=0)  # Forward pass
            loss = self.compute_loss(outputs, targets)
            loss.backward()  # Backward pass to compute gradients
...
def train_model(adapter_class, lr=0.01, weight_decay=0.01, epochs=100):
    adapter = adapter_class()
    adapter.random_init_all_params()

    data, targets = adapter.generate_synthetic_data()
    adapter.train_step(data, targets, lr, weight_decay, epochs)
...
def main():
    ...
    train_model(adapter_class, lr=0.01, weight_decay=0.01, epochs=100)
