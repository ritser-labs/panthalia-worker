...
async def wait_for_health(url, timeout=300):
...
async def safe_update_master_state(db_adapter, job_id, update_fn):
    """
    Safely update the master_state_json for a given job, so that 
    state transitions are immediately persisted to the DB.

    :param db_adapter: The DB adapter client (your custom class).
    :param job_id: The job ID in the DB.
    :param update_fn: A function that takes a dict (the current master_state_json)
                      and modifies it in-place. That updated dict is then written back.
    """
    state_data = await db_adapter.get_master_state_for_job(job_id)
    update_fn(state_data)
