...
async def wait_for_result(db_adapter, plugin, sot_url, job_id: int, task_id: int) -> dict:
    ...
    times in a row, re-create a new Task ID.
    """
    missing_count = 0
    ...
    while True:
        ...
        if not task:
            ...
            if missing_count >= TASK_MISSING_THRESHOLD:
                ...
                if not new_task_id:
                    # If re-creation fails or job is set inactive => no result
                    return {}
                task_id = new_task_id
                missing_count = 0
            else:
...
###############################################################################
# Retrieve input_url from SOT ( /get_batch ), with retries
###############################################################################
...
###############################################################################
# Create a DB Task + bid for this iteration’s training, storing last_task_creation_time
###############################################################################
async def submit_task_with_persist(db_adapter, job_id: int, iteration_number: int, params_str: str):
    ...
    """
    import datetime
    from ..common import MAX_SUBMIT_TASK_RETRY_DURATION
    ...
    while True:
        ...
        try:
            # For simplicity, just use target_price=1 or the job's actual price
            # For multi-bid logic, see main_logic's get_bid_price or pass it in
            price = 1  
            created_items = await db_adapter.create_bids_and_tasks(
                job_id, 1, price, params_str, None
            )
            if created_items and len(created_items) > 0:
                # store last_task_creation_time
                state_data = await db_adapter.get_master_state_for_job(job_id)
...
###############################################################################
# If a Task was forcibly deleted from DB, re-create it
###############################################################################
async def _handle_recreate_missing_task(db_adapter, plugin, sot_url, job_id: int, old_task_id: int):
    """
    1) Find iteration referencing old_task_id,
    2) revert from 'pending_wait_for_result' => 'pending_submit_task',
    3) create a new DB task,
    4) store new task_id in iteration_state,
    5) if we exceed MAX_RECREATES_PER_ITERATION => set job inactive
    """
    ...
    # Count how many times we re-created for that iteration
    ...
    if cur_count >= MAX_RECREATES_PER_ITERATION:
        logger.error(f"[_handle_recreate_missing_task] iteration={iteration_num} used up re-creates => job inactive.")
    ...
    # If no learning_params, re-fetch
    ...
    if not input_url:
        logger.error(f"[recreate_missing_task] iteration={iteration_num} missing input_url => cannot proceed.")
        return 0
...
###############################################################################
# Utility to sign with the Master’s private key from args
###############################################################################
